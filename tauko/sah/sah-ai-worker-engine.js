...
function getSearchDepth(level) {
    ...
    const depths = {
        ...
        3: 3,  // Klub igralec
        4: 4,  // Srednji
        5: 5,  // Napredni (Worker enabled)
        6: 6   // Expert (Worker enabled)
    };
...
// Worker verzija - preimenuj da ne konfliktira
async function Predlagaj_Potezo_Worker(p_in_stanje_FEN, p_in_max_sekund, p_callback) {
    ...
    // WEB WORKER VERZIJA: Brez omejitev - lahko računa L1-L6
    // Worker teče v ločeni niti, ne blokira UI!
    const maxLevel = 6;
    ...
    for (let level = 1; level <= 6; level++) {
        ...
        // Minimalna pavza v Worker-ju (samo 10ms - ne rabi več)
        if (level < 6) {
...
// ==========================================
// EXPORTS - WORKER VERZIJA
// ==========================================

// Web Worker export (self scope) - samo za Worker, omogoča L1-L6
self.Predlagaj_Potezo_Worker = Predlagaj_Potezo_Worker;
self.oceniPozicijo = oceniPozicijo;
self.minimax = minimax;
self.getSearchDepth = getSearchDepth;
self.shouldMakeMistake = shouldMakeMistake;

console.log('✅ Worker Engine loaded - L1-L6 enabled');
